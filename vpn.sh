#!/bin/bash
# ==================================================
# SISIPAN 1: KONFIGURASI BOT & FUNGSI LAPORAN
# ==================================================
...
clear
# REPO    
    REPO="https://raw.githubusercontent.com/galihbengkulu777/ppl/main/"
...
clear
izinsc="https://raw.githubusercontent.com/galihbengkulu777/ppl/main/ijin"
# USERNAME
rm -f /usr/bin/user
...
bash -c "$(curl -L https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" @ install -u www-data --version $latest_version
    ...
    sudo curl -fsSL https://raw.githubusercontent.com/galihbengkulu777/ppl/main/config/nginx.conf -o /etc/nginx/nginx.conf
...
wget -q https://raw.githubusercontent.com/galihbengkulu777/ppl/main/config/fv-tunnel && chmod +x fv-tunnel && ./fv-tunnel
...
print_install "Memasang modul SlowDNS Server"
    ...
    # 👇👇 TAMBAHAN BYPASS UNTUK BOT TELEGRAM 👇👇
    if [[ -n "$AUTO_NS" ]]; then
        echo "Bypass input NS untuk bot Telegram..."
        
        # 1. Hapus semua perintah 'read -p' di dalam script nameserver agar tidak nyangkut
        sed -i '/read -p/d' /tmp/nameserver
        
        # 2. Suntikkan variabel NS_DOMAIN secara paksa ke baris ke-2 script
        # (Kita masukkan beberapa nama variabel umum karena kita tidak tahu pasti nama variabel di dalam file nameserver-nya)
    ...
    # Jalankan script tanpa interaksi dan simpan log-nya
...
wget -q -O /etc/default/dropbear https://raw.githubusercontent.com/galihbengkulu777/ppl/main/config/dropbear.conf
...
wget -q https://raw.githubusercontent.com/galihbengkulu777/ppl/main/udp-custom/udp-custom.sh
...
#OpenVPN
wget https://raw.githubusercontent.com/galihbengkulu777/ppl/main/config/openvpn  chmod +x openvpn && ./openvpn
...
# Ganti Banner
wget -O /etc/kyt.txt https://raw.githubusercontent.com/galihbengkulu777/ppl/main/files/issue.net
...
function install_zivpn() {
    ...
    # Membuat Config JSON Default (Port 5667)
...
EOF
    ...
    # 5. MEMBUAT SCRIPT AUTO-SYNC (Anti-Error Jam 12 Malam)
...
EOF
    ...
    # 6. KONFIGURASI SYSTEMD (Anti-Bengong saat Reboot)
...
EOF
    ...
    # 7. ROUTING IPTABLES (Metode REDIRECT yang Paling Stabil)
    echo -e "Setup Iptables Routing..."
    # Bersihkan routing DNAT lama jika ada
    ...
    # Simpan permanen agar aman saat reboot
...
function instal(){
    ...
    base_package
    update_progress 15 "CEK PROSES INSTAL DENGAN KETIK DI VPS : tail -f /root/install_ssh.log"
    make_folder_xray
    ...
    # Bagian ini akan menginstall Xray versi TERBARU dulu
    ...
    # --- PROSES DOWNGRADE DISISIPKAN DI SINI ---
